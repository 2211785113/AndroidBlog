...
类实现Serializable就代表类序列化。对象的序列化是指
...
共同点：都是抽象的，即把多个事物共性的内容抽取出来，都不能实例化。

区别：最简单的区别就是关键字，抽象类用extends继承，接口用implements实现；抽象类有构造函数，接口没有；接口解决了Java中只能单继承的痛点，可以多实现，而抽象类不能，接口比抽象类抽象级别更高；抽象类中可以有实例域和具体方法，接口中不可以，接口中只有抽象方法和常量(此为本质区别)。

注意一点：接口是完全抽象，用于功能拓展，比抽象类更加宽泛；而抽象类类似于下定义，比如abstractList是有序的，那么它的子类也是有序的，为部分抽象，有局限。抽象类是继承，更多代表is-a关系；接口是实现，更多代表like-a关系。（个人认为这条并不准确）

项目中例子：

- recyclerview通用万能适配器BaseAdapter设为抽象类，抽象出每个adapter子类共性的部分。因为要定义实例域，所以用抽象类。
- recyclerview多itemtype，定义一个multiItemBuilder接口或者抽象类，里面定义getLayoutId和getItemType方法。因为只定义方法，所以抽象类和接口都可以。
- 抽象类和接口都能抽象动作：例如Human/Man/Woman共同的say方法。因为只抽象方法，所以抽象类和接口都可以，同上。

源码中经典例子：

- List，Map(看集合的时候还需要多加体会)
...
为什么：允许在编译期间不知道类名称、接口名称、字段、方法的情况下在运行时检查类、接口、字段、方法。
...
用途/后果：
用途：第一利用反射分析类，调用类的属性、构造器、方法，不在运行时也可以，例如捕获异常；第二在运行时分析类，例如访问不可变对象中的私有成员。

项目中应用：
https://blog.csdn.net/u010002184/article/details/90755625
https://blog.csdn.net/hgdzw/article/details/87697045
序列化时：运行时分析类，查看对象。
心神学堂：获取状态栏和导航栏的高度；搜索框烟火控件通过反射获取Edittext类中光标的坐标。
WindowUtil：
获取状态栏StatusBar的高度：
https://blog.csdn.net/xu_coding/article/details/80387893
其中一种方法：反射。
获取导航栏NavigationBar的高度：
https://www.jianshu.com/p/c1f2a3849d6e
其中一种方法：反射。
利用反射机制获得model对象的引用，用包名+type作为token直接找到对应的Model。
...
1)不捕获变量：方法实现会被提取到一个与之具有相同签名的静态方法中，这个静态方法和lambda表达式位于同一个类上。
2)捕获变量：依然会被提取到一个静态方法中，被捕获的变量会同正常的参数一样传入到这个方法中。

**说说你对接口的理解**

接口是抽象的概念，抽象动作，和类是like-a的关系。
一个类可以实现多个接口，接口里只有静态变量和抽象方法。静态变量在编译时已确定，如果调用出错会报编译错误，引用时直接通过类名或接口名调用。
接口实现多态的行为。多态期望的作用是任何使用父类对象的地方都能够透明的使用子类。
接口不能被实例化。

**匿名内部类和内部类有什么区别？**

Java 8新特性：lamada表达式和stream API✔️
lamada表达式的演变✔️
...
* 编译器调用add知道参数的类型，编译器可进行检查，避免插入错误类型的对象。
...
类型变量的限定：限定类型用“&”分隔；类型变量用“，”分隔。T extends Comparable & Serializable。

泛型代码和虚拟机：

类型擦除：限定类型变量的第一个，如果没有限定类型用Object。

翻译泛型表达式：编译器将泛型方法翻译成两条虚拟机指令：

* 先调用方法擦除返回类型，编译器再插入强制类型转换。
...
项目中使用：list/map集合，接口，网络请求。mvp-x。

### 10.集合
**分类**

**常用集合框架实现原理**（之后自己看源码并出一个专题）

http://wiki.jikexueyuan.com/project/java-collection/hashmap.html
* ArrayList，LinkedList实现原理；✔️
* HashMap，LinkedHashMap，ConcurrentHashMap实现原理；✔️
* Hashtable实现原理；✔️
* HashSet，LinkedHashSet实现原理；✔️
* LinkedHashMap与LRUcache；✔️
* HashSet与HashMap的比较；✔️

**ArrayList和LinkedList的区别**

https://www.cnblogs.com/Jacck/p/8034900.html
- ArrayList基于数组实现；LinkedList基于双向链表实现。
- 随机访问元素，大多数情况下ArrayList比LinkedList快，因为ArrayList是直接通过索引获取，LinkedList要移动指针，比如有一种情况特殊，列表反转Collections.reverse();LinkedList比ArrayList快，ArrayList需要挪动大量的元素，但是LinkedList只需要将指针重新指向即可。
...
* HashMap：线程不安全
* HashTable与Vector：同步，线程安全。 并发访问用ConcurrentHashMap
...
https://blog.csdn.net/QQQQQQ654/article/details/70144050

Java的多线程机制？
https://www.cnblogs.com/keyi/p/7173036.html

线程中sleep方法和wait方法的区别：
https://www.cnblogs.com/linkstar/p/6043846.html
...
wait：是Object的方法，wait是对象锁，锁定方法不让继续执行，当执行notify方法后就会继续执行。
sleep：是Thread的方法，sleep是使线程睡眠，让出cpu，结束后自动继续执行
