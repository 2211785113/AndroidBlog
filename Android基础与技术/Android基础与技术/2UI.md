...
* View动画：四种简单变换，平移旋转缩放透明度。作用对象为View或ViewGroup。ViewGroup指定动画控制子元素的动画效果；Activity或Fragment实现切换动画效果。
* 属性动画：可以做出更多的动画效果。作用对象为任何对象包括View，也可以没有对象。实现匀速动画和非匀速动画。例子：给一个Button按钮实现宽度从0到500px的动画效果。
* 帧动画：仿美团京东加载效果。代码：FrameAnimation
...
收到通知后存储一个值判断该条通知的状态：为true则不显示，为false即没有读过则显示该条通知。关键：每一条通知都要区分开来，可以用time时间戳+account用户+uuid32位随机码来区分。
