...
- 产品类Product：
- 抽象建造者Builder：规范产品的组建。
- 具体建造者ConcreteBuilder：实现抽象 Builder 类定义的所有方法，并且返回一个组建好的对象。
- 指挥者Director：负责安排已有模块的顺序，统一组建过程，然后通知Builder开始构建。
- 客户端：客户端调用指挥者。
...
- Retrofit源码：new一个具体建造者Retrofit.Builder()设置/配置一系列参数后调用build()方法生成retrofit对象。
